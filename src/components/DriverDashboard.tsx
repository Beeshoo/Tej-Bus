...
const DriverDashboard: React.FC<DriverDashboardProps> = ({ driver, onLogout, onNavigate }) => {
  const [activeTab, setActiveTab] = useState<'trips' | 'ratings' | 'profile' | 'scanner'>('trips');
  const [tickets, setTickets] = useState<Ticket[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [scanResult, setScanResult] = useState<{ success: boolean; ticket?: Ticket; passenger?: User; message: string } | null>(null);
  ...
  useEffect(() => {
    if (activeTab === 'scanner') {
      ...
      const onScanSuccess = async (decodedText: string) => {
        // الروابط لدينا بصيغة: https://tajbus.com/verify/TAJ-XXXXXX
        ...
        if (ticketId) {
          ...
          try {
            ...
            if (ticket) {
              const passenger = await BackendAPI.getUserById(ticket.userId);
              setScanResult({
                success: true,
                ticket,
              ...
              setScanResult({ success: false, message: 'عذراً، هذه التذكرة غير موجودة في النظام ❌' });
            }
          } catch (err) {
            setScanResult({ success: false, message: 'حدث خطأ أثناء فحص التذكرة ❌' });
          } finally {
            setLoading(false);
          }
        }
      ...
      scanner.render(onScanSuccess, (err) => {
        // تجاهل أخطاء الفشل المستمر في البحث عن كود
      });
  ...
  const refreshData = async () => {
    try {
      ...
      console.error("خطأ في تحديث بيانات الكابتن", err);
      setError("فشل في تحميل الرحلات المسندة.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col md:flex-row font-tajawal text-white">
      ...
      <aside className="w-full md:w-72 bg-slate-900 border-e border-slate-800 flex flex-col shrink-0">
        <div className="p-8 border-b border-slate-800">
          <div className="flex items-center gap-3 mb-2">
            <Logo className="w-10 h-10" />
            <h1 className="text-xl font-black tracking-tight">تاج باص <span className="text-emerald-500 text-[10px] block font-black uppercase tracking-widest mt-1 opacity-80">بوابة الكابتن</span></h1>
          </div>
        ...
        <nav className="flex-grow p-4 space-y-2">
          {[
            ...
            { id: 'scanner', label: 'مسح تذاكر الركاب', icon: '📷' },
        ...
        <header className="flex justify-between items-center mb-10">
          <div>
            <h2 className="text-3xl font-black text-white">
              ...
              {activeTab === 'scanner' && 'التحقق من التذاكر'}
        ...
        <div className="animate-fadeIn">
          {activeTab === 'scanner' && (
            <div className="max-w-xl mx-auto space-y-6">
              {!scanResult ? (
                <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-800 text-center shadow-2xl overflow-hidden">
                  <div className="mb-8">
                    <h3 className="text-2xl font-black text-white mb-2">امسح الكود ضوئياً</h3>
                    <p className="text-slate-500 font-medium small">قم بتوجيه الكاميرا نحو تذكرة الراكب للتحقق من صلاحيتها</p>
                  </div>
                  
                  <div id="qr-reader" className="overflow-hidden rounded-3xl border-4 border-emerald-500/20"></div>
                  
                  <div className="mt-8 p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 text-emerald-500 text-xs font-bold leading-relaxed">
                    سيتم التعرف على التذاكر الرسمية لشركة تاج باص فقط بشكل تلقائي
                  </div>
                </div>
              ) : (
                <div className="bg-slate-900 rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl animate-popIn">
                  <div className={`p-8 text-center ${scanResult.success ? 'bg-emerald-600' : 'bg-red-600'}`}>
                    <div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-4 text-white text-4xl">
                      {scanResult.success ? '✅' : '❌'}
                    </div>
                    <h3 className="text-2xl font-black text-white">{scanResult.message}</h3>
                  </div>
                  
                  {scanResult.success && scanResult.ticket && (
                    <div className="p-10 space-y-8">
                      <div className="flex items-center gap-6">
                        <div className="w-20 h-20 bg-slate-800 rounded-3xl border-2 border-slate-700 flex items-center justify-center text-3xl overflow-hidden shrink-0">
                          {scanResult.passenger?.photoUrl ? (
                            <img src={scanResult.passenger.photoUrl} alt="Passenger" className="w-full h-full object-cover" />
                          ) : '👤'}
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">بيانات الراكب</p>
                          <h4 className="text-2xl font-black text-white">{scanResult.passenger?.name || 'مسافر كريم'}</h4>
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-6 bg-slate-950/50 p-6 rounded-2xl border border-slate-800">
                        <div>
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">المسار</p>
                          <p className="font-black text-white">{scanResult.ticket.from} ← {scanResult.ticket.to}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">التاريخ</p>
                          <p className="font-black text-white">{scanResult.ticket.date}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">المقاعد</p>
                          <p className="font-black text-emerald-500">{scanResult.ticket.selectedSeats.join(', ')}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">رقم التذكرة</p>
                          <p className="font-black text-white text-xs">{scanResult.ticket.id}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="p-6 bg-slate-950/50 flex gap-4">
                    <button 
                      ...
                      className="flex-1 py-4 bg-emerald-600 text-slate-950 font-black rounded-2xl hover:bg-emerald-500 transition-all shadow-xl shadow-emerald-500/20"
                    >
                      فحص تذكرة أخرى
                    </button>
                  </div>
                    ...
                    {tickets.map(t => (
                      <tr key={t.id} className="hover:bg-emerald-500/5 transition-colors">
                        ...
                        <td className="p-6 text-sm font-medium text-slate-400">
                          {t.date} 
                          <span className="text-[10px] block opacity-50">{to12Hour(t.departureTime)}</span>
                        </td>
                        <td className="p-6">
          ...
          {activeTab === 'profile' && (
            <div className="max-w-2xl bg-slate-900 rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl">
              <div className="h-32 bg-gradient-to-r from-emerald-600 to-emerald-900 relative">
                <div className="absolute -bottom-12 start-12 w-24 h-24 bg-slate-900 rounded-3xl border-4 border-slate-950 flex items-center justify-center text-4xl shadow-xl overflow-hidden">
                  {driver.photoUrl ? (
                    <img src={driver.photoUrl} alt={driver.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
                    '👨‍✈️'
