...
const BookingFlow: React.FC<BookingFlowProps> = ({ onComplete, userId }) => {
  // --- تعريف حالات عملية الحجز (Booking Process State) ---
  const [step, setStep] = useState(1); // الخطوة الحالية من 1 إلى 5
  const [occupiedSeats, setOccupiedSeats] = useState<number[]>([]); // المقاعد المحجوزة مسبقاً في هذه الرحلة
  const [booking, setBooking] = useState<BookingState>({
    from: '', to: '', busType: BusType.STANDARD, date: '', departureTime: '', arrivalTime: '', selectedSeats: []
  });

  // تحديث قائمة المقاعد المحجوزة بمجرد اكتمال بيانات الرحلة (التاريخ والوقت والمسار)
  useEffect(() => {
  ...
  const handleNext = () => setStep(step + 1); // الانتقال للخطوة التالية
  const handleBack = () => setStep(step - 1); // العودة للخطوة السابقة
  
  // دالة اختيار أو إلغاء اختيار مقعد
  const toggleSeat = (seat: number) => {
    if (occupiedSeats.includes(seat)) return; // منع اختيار المقاعد المحجوزة بالفعل
    setBooking(prev => ({
      ...prev,
  ...
  // معالجة الدفع الناجح وإرسال الطلب النهائي للباك-إند
  const handlePaymentSuccess = async (paymentId: string) => {
    try {
      ...
      if (result.success && result.ticket) {
        onComplete(result.ticket); // إشعار التطبيق الرئيسي باكتمال الحجز لتوليد التذكرة
      }
    } catch (err: any) {
      throw err;
    }
  ...
  // دالة التحقق من إمكانية الانتقال للخطوة التالية بناءً على البيانات المدخلة
  const canGoNext = () => {
    switch(step) {
      case 1: return booking.from && booking.to && booking.from !== booking.to && booking.date; // التأكد من اختيار المدن والتاريخ
      case 2: return booking.departureTime !== ''; // التأكد من اختيار وقت التحرك
      case 3: return true; // نوع الحافلة له قيمة افتراضية
      case 4: return booking.selectedSeats.length > 0; // التأكد من اختيار مقعد واحد على الأقل
      default: return true;
    }
  };
